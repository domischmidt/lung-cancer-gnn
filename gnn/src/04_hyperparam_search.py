...
def train_and_evaluate(
    ...
    for epoch in range(1, search_epochs + 1):
        ...
        for i in range(0, train.size(0), config["batch_size"]):
            ...
            neg = generate_negatives_type_aware(
                batch,
                rel_ranges_cpu,
                config["neg_ratio"],
            )
            ...
            del z, pos_score, neg_score, loss, pos_loss, neg_loss, batch, neg

        if epoch % 20 == 0 or epoch == 1 or epoch == search_epochs:
            avg_loss = epoch_loss / max(n_batches, 1)
            print(f"      epoch {epoch:>3d}/{search_epochs} loss={avg_loss:.4f}")
    ...
    for i in range(val_sample.size(0)):
        ...
        del scores, all_ents, h_rep, r_rep

    overall = metrics_from_ranks(all_ranks)

    gda_ranks = per_rel.get(GDA_KEY, [])
    gda = metrics_from_ranks(gda_ranks) if len(gda_ranks) >= 3 else None

    del model, optimizer, z, ei, et
    cleanup_cuda()
...
def fig_optimization_history(study, fig_dir):
    ...
    if not trials:
        print("  [SKIP] No valid completed trials for optimization history")
        return
    ...
    ax.plot(
        range(len(best_so_far)),
        best_so_far,
        color="#c44e52",
        linewidth=2,
        label="Best so far",
    )
    ax.set_xlabel("Valid trial", fontsize=11)
    ...
    plt.tight_layout()
    fig.savefig(
        fig_dir / "hyperparam_optimization_history.png",
        dpi=200,
        bbox_inches="tight",
    )
    plt.close(fig)

    print("  -> figs/hyperparam_optimization_history.png")
...
def fig_param_importance(study, fig_dir):
    ...
    try:
        ...
        print(f"  [SKIP] Could not compute parameter importances: {e}")
        return

    if not importances:
        print("  [SKIP] No parameter importances available")
        return
    ...
    for i, v in enumerate(values[::-1]):
        ax.text(v + max_value * 0.02, i, f"{v:.3f}", va="center", fontsize=9)
    ...
    plt.tight_layout()
    fig.savefig(
        fig_dir / "hyperparam_importance.png",
        dpi=200,
        bbox_inches="tight",
    )
    plt.close(fig)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Epochs stored in best_config.json for final 05_train_rgcn.py run",
    )
    ...
    print("=" * 70)
    print(
        f"04_hyperparam_search.py "
        f"({args.n_trials} trials, {args.search_epochs} epochs each)"
    )
    print("=" * 70)
    print(f"Device: {DEVICE}")
    print("Objective: maximize GDA -> Disease MRR on validation set")
    ...
    try:
        ...
        print("  -> best_config.json (defaults)")
        return
    ...
    def objective(trial):
        config = {
            ...
            "neg_ratio": trial.suggest_categorical("neg_ratio", [5, 10]),
        ...
        try:
            ...
            print(
                f"    Trial {trial.number:>3d}  OOM skipped  "
                f"layers={config['num_layers']} bases={config['num_bases']} "
                f"hidden={config['hidden_dim']} "
                f"neg={config['neg_ratio']} batch={config['batch_size']} "
                f"({dt:.0f}s)"
            )
        ...
        print(
            ...
            f"lr={config['lr']:.5f} neg={config['neg_ratio']} "
    ...
    valid_results = [
        ...
        if r.get("status") == "complete" and r.get("objective") is not None
    ]

    if not valid_results:
        print("\n  WARNING: No valid trials completed. Saving stable default config.")
        ...
        best_valid = max(valid_results, key=lambda r: r["objective"])
        best_config = dict(best_valid["config"])
        best_config["epochs"] = args.final_epochs

        best_trial_number = best_valid["trial"]
        best_value = best_valid["objective"]

        print(f"\n  Best valid trial: {best_trial_number}")
        print(f"  Best objective MRR: {best_value:.4f}")
        print("  Best config:")
    ...
    print("  -> hyperparam_search_results.json")
    ...
    print("Run 05_train_rgcn.py to train with the best configuration.")
    print("=" * 70)
