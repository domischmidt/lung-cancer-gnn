...
# Default config; overridden by best_config.json if available
HIDDEN_DIM = 128
NUM_LAYERS = 4
NUM_BASES = 6
DROPOUT = 0.2
...
if _config_path.exists():
    ...
    EPOCHS = min(int(_cfg.get("epochs", EPOCHS)), 100)
    print(f"[Config] Loaded best_config.json: hidden={HIDDEN_DIM}, layers={NUM_LAYERS}, "
          f"bases={NUM_BASES}, dropout={DROPOUT}, lr={LR}, neg={NEG_RATIO}, "
          f"batch={BATCH_SIZE}, epochs={EPOCHS}")

SEEDS = [456, 789, 1337]
ABLATION_SEEDS = [456, 789, 1337]
N_EVAL_SAMPLES = 500
N_PERMUTATIONS = 500
...
def main():
    print("=" * 70)
    print(f"09_full_evaluation.py (type-aware, {NUM_LAYERS} layers)")
    print("=" * 70)
